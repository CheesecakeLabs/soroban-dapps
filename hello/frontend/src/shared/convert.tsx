...
export function bigNumberToI128(value: BigNumber): SorobanClient.xdr.ScVal {
  ...
  const hi = new xdr.Int64(
    bigNumberFromBytes(false, ...Array.from(padded.slice(4, 8))).toNumber(),
    bigNumberFromBytes(false, ...Array.from(padded.slice(0, 4))).toNumber()
  );
  const lo = new xdr.Uint64(
    bigNumberFromBytes(false, ...Array.from(padded.slice(12, 16))).toNumber(),
    bigNumberFromBytes(false, ...Array.from(padded.slice(8, 12))).toNumber()
  );
