...
/**
 * Invoke a method on the liquidity-poo contract.
 *
 ...
 */
export async function invoke<R extends ResponseTypes = undefined, T = string>(contractId: string, args: InvokeArgs<R, T>): Promise<R extends undefined ? T : R extends "simulated" ? Simulation : R extends "full" ? SomeRpcResponse : T>;
export async function invoke<R extends ResponseTypes, T = string>(contractId: string, {
  method,
  args = [],
  fee = 100,
  responseType,
