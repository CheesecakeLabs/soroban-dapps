...
LIQUIDITY_POOL_ID=$(<"${DATA_DIR}/${AMM_DEPLOY_OUTPUT_FILE}-out")

echo "Generating bindings"
soroban contract bindings typescript --wasm $ASSET_WASM --network $NETWORK_NAME --contract-id $A_CONTRACT_ID_UPDT --output-dir ".soroban/contracts/token-a" --overwrite
soroban contract bindings typescript --wasm $ASSET_WASM  --network $NETWORK_NAME --contract-id $B_CONTRACT_ID_UPDT --output-dir ".soroban/contracts/token-b" --overwrite
soroban contract bindings typescript --wasm $AMM_WASM --network $NETWORK_NAME --contract-id $LIQUIDITY_POOL_ID --output-dir ".soroban/contracts/liquidity-pool" --overwrite
