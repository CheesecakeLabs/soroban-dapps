...
use crate::storage_types::{DataKey, BALANCE_BUMP_THREASHOLD, BALANCE_BUMP_AMOUNT};

use crate::balance::{execute_withdraw, calculate_yield, calculate_withdraw};
use crate::validation::{is_deposit_valid,is_initialized,has_achieved_completion,has_not_active_deposit,has_active_deposit};
use crate::data::{get_asset,get_deposit_amount, set_user_position, get_admin, get_deposit_timestamp, get_term};

pub trait CertificateOfDepositsTrait {

    fn initialize(env: Env, admin:Address,asset: Address, term: u64, compound_step: u64, yield_rate: u64, min_deposit: i128, penalty_rate: u64);
...
impl CertificateOfDepositsTrait for CertificateOfDeposits {
    ...
    fn initialize(env: Env, admin:Address,  asset: Address,  term: u64, compound_step: u64, yield_rate: u64, min_deposit: i128, penalty_rate: u64) { 
        ...
        env.storage().persistent().set(&DataKey::CompoundStep, &compound_step);
    ...
    fn extend_contract_validity(env: Env){
        ...
        //
        env.storage().persistent().bump(&DataKey::Admin, BALANCE_BUMP_THREASHOLD, BALANCE_BUMP_AMOUNT);
        env.storage().persistent().bump(&DataKey::Asset, BALANCE_BUMP_THREASHOLD, BALANCE_BUMP_AMOUNT);
        env.storage().persistent().bump(&DataKey::Term, BALANCE_BUMP_THREASHOLD, BALANCE_BUMP_AMOUNT);
        env.storage().persistent().bump(&DataKey::CompoundStep, BALANCE_BUMP_THREASHOLD, BALANCE_BUMP_AMOUNT);
        env.storage().persistent().bump(&DataKey::YieldRate, BALANCE_BUMP_THREASHOLD, BALANCE_BUMP_AMOUNT);
        env.storage().persistent().bump(&DataKey::MinDeposit, BALANCE_BUMP_THREASHOLD, BALANCE_BUMP_AMOUNT);
        env.storage().persistent().bump(&DataKey::PenaltyRate, BALANCE_BUMP_THREASHOLD, BALANCE_BUMP_AMOUNT);
        ...
        let asset_address: Address = get_asset(&env);
        let token_client = token::Client::new(&env, &asset_address);
        token_client.approve(&admin_address,&env.current_contract_address(), &allowance_amount, &expiration_ledger );
