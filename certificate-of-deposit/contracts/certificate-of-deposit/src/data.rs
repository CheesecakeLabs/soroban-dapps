...
use crate::storage_types::{DataKey,DepositData, BALANCE_BUMP_THREASHOLD, BALANCE_BUMP_AMOUNT};
...
pub fn get_deposit_amount(env: &Env, address: Address) -> i128 {
    if let Some(user) = env.storage().persistent().get::<_, DepositData>(&DataKey::Deposit(address)) {
        user.amount
    } else {
        0
    }
...
pub fn clear_user_position(env: &Env, address: Address){
    env.storage().persistent().remove(&DataKey::Deposit(address));
}
...
pub fn set_user_position(env: &Env, address: Address,amount: i128, timestamp:u64){
    ...
    env.storage().persistent().set(&DataKey::Deposit(address.clone()), &deposit_data); 
    env.storage().persistent().bump(&DataKey::Deposit(address), BALANCE_BUMP_THREASHOLD, BALANCE_BUMP_AMOUNT); 
